...
public static class Sqlite<T> where T : SqliteTable
{
    ...
    /// </summary>
    /// <param name="where">
    ///     The WHERE clause of the SQL squery to execute. Will be preceded
    ///     by "SELECT * FROM {type} " in the actual query
    /// </param>
    ...
    public static IAsyncEnumerable<T> Select(string? where = null, bool onlyRowId = false)
    {
        ...
        (
            tableMapping,
            $"SELECT RowId,* FROM {TableMapping<T>.TableName} {where}",
            onlyRowId
        );
        return enumerable;
    }
    ...
    /// <summary>
    ///     Inserts a range of SqliteTable entities into the given Sqlite database table and returns the number of inserted
    ///     entities
    /// </summary>
    public static ValueTask<long> Insert(params T[] entities)
    {
        return Insert(entities.ToAsyncEnumerable()).LongCountAsync();
    }
    ...
    /// <summary>
    ///     Updates a range of SqliteTable entities in the given Sqlite database table and returns the number of updated
    ///     entities
    /// </summary>
    public static ValueTask<long> Update(params T[] updatedEntities)
    {
        return Update(updatedEntities.ToAsyncEnumerable()).LongCountAsync();
    }

    /// <summary>
    ///     Updates a range of SqliteTable entities in the given Sqlite database table and returns an enumeration of updated
    ///     entities
    /// </summary>
    ...
    /// <summary>
    ///     Deletes a range of SqliteTable entities in the given Sqlite database table and returns the number of deleted
    ///     entities
    /// </summary>
    public static ValueTask<long> Delete(params T[] toDelete)
    {
        return Delete(toDelete.ToAsyncEnumerable());
    }

    /// <summary>
    ///     Deletes the corresponding Sqlite database table rows for a given enumeration of entities, and returns the number of
    ...
    /// </summary>
    /// <param name="where">
    ///     The WHERE clause of the SQL query to execute. Will be preceded
    ///     by "SELECT COUNT(*) FROM {type} " in the actual query
    /// </param>
    /// <returns></returns>
