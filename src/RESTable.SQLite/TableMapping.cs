...
public class TableMapping : ISelector<TableMapping>
{
    ...
    static TableMapping()
    {
        TableMappingByType = new ConcurrentDictionary<Type, TableMapping>();
    }
    ...
    {
        return TableMappingByType.SafeGet(type);
    }
    ...
    public async IAsyncEnumerable<SqlColumn> GetSqlColumns()
    {
        await foreach (var row in TableInfoQuery.GetRows().ConfigureAwait(false))
        {
            ...
            yield return new SqlColumn(name, type.ParseSqlDataType());
        }
    }

    private void ReloadColumnNames(ColumnMappings columnMappings)
    {
        SqlColumnNames = MakeColumnNames(columnMappings);
    }
    ...
    internal async Task Update()
    {
        ...
        await foreach (var column in GetSqlColumns().Where(column => !columnNames.Contains(column.Name)).ConfigureAwait(false))
            ...
            (
                this,
                new ClrProperty(column.Name, column.Type.ToClrTypeCode()),
                column
            );
        ReloadColumnNames(columnMappings);
    ...
    {
        await DropTableQuery.ExecuteAsync().ConfigureAwait(false);
    }

    // ReSharper disable once ConstantNullCoalescingCondition
    private string GetCreateTableSql()
    {
        return $"CREATE TABLE {TableName} ({(ColumnMappings ?? GetDeclaredColumnMappings()).ToSql()});";
    }
    ...
    private ColumnMappings GetDeclaredColumnMappings()
    {
        var columnMappings = ClrClass
            ...
            (
                this,
                property,
                new SqlColumn(property.MemberAttribute?.ColumnName ?? property.Name, property.Type.ToSqlDataType())
            ));
