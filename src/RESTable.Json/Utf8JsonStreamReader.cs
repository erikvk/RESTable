...
namespace RESTable.Json
{
    public ref struct Utf8JsonStreamReader
    {
        ...
        private void MoveNext()
        {
            ...
            var data = newSegment.Buffer.Memory.Span;

#if !NETSTANDARD2_0
            _lastSegmentEndIndex = _stream.Read(data);
#else
            var bytes = data.ToArray();
            _lastSegmentEndIndex = _stream.Read(bytes, 0, bytes.Length);
#endif

            _isFinalBlock = _lastSegmentEndIndex < newSegment.Buffer.Memory.Length;
            _jsonReader = new Utf8JsonReader(new ReadOnlySequence<byte>(_firstSegment, _firstSegmentStartIndex, _lastSegment, _lastSegmentEndIndex), _isFinalBlock,
        ...
        public T Deserialize<T>(JsonSerializerOptions? options = null)
        {
            ...
            var newJsonReader =
                new Utf8JsonReader(
                    new ReadOnlySequence<byte>(firstSegment!, firstSegmentStartIndex, _lastSegment!, _lastSegmentEndIndex).Slice(tokenStartIndex, _jsonReader.Position), true,
                    default);
