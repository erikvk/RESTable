...
{
    /// <summary>
    /// A static class for finding registered RESTable resources
    /// </summary>
    public static class Resource
    {
        ...
        /// <summary>
        /// Gets all registered RESTable resources
        /// </summary>
        public static IEnumerable<IResource> All => RESTableConfig.Resources;

        /// <summary>
        /// Gets all registered RESTable resources that were claimed by the given 
        ...
        public static IResource Find(string searchString, ResourceKind kind = ResourceKind.All)
        {
            ...
            if (!RESTableConfig.ResourceFinder.TryGetValue(searchString, out var resource))
        ...
        public static bool TryFind(string searchString, out IResource resource, out Error error, ResourceKind kind = ResourceKind.All)
        {
            ...
            error = null;
            if (!RESTableConfig.ResourceFinder.TryGetValue(searchString, out resource))
            {
        ...
        public static IResource[] SafeFindMany(string searchString)
        {
            switch (searchString.Count(i => i == '*'))
            {
                ...
                case 1:
                    ...
                    return RESTableConfig.ResourceByName
        ...
        {
            return All.FirstOrDefault(r => string.Equals(r.Type.GetRESTableTypeName(), typeName, StringComparison.CurrentCultureIgnoreCase));
        }

        /// <summary>
        /// Gets the resource for a given type, or throws an UnknownResource exception if there is no such resource
        /// </summary>
        public static IResource Get(Type type) => RESTableConfig.ResourceByType.SafeGet(type) ?? throw new UnknownResource(type.GetRESTableTypeName());

        /// <summary>
        /// Gets the resource for a given type or returns null if there is no such resource
        /// </summary>
        public static IResource SafeGet(Type type) => RESTableConfig.ResourceByType.SafeGet(type);

        /// <summary>
        /// Finds a resource by name (case insensitive) and throws an UnknownResource exception
        /// if no resource is found.
        /// </summary>
        public static IResource Get(string name) => RESTableConfig.ResourceByName.SafeGet(name) ?? throw new UnknownResource(name);

        /// <summary>
        /// Finds a resource by name (case insensitive) and returns null
        /// if no resource is found
        /// </summary>
        public static IResource SafeGet(string name) => RESTableConfig.ResourceByName.SafeGet(name);

        #endregion
    }

    /// <summary>
    /// A static generic class for manually getting RESTable resources by type
    /// </summary>
    public static class Resource<T> where T : class
    {
        ...
        /// </summary>
        public static IResource<T> Get => RESTableConfig.ResourceByType.SafeGet(typeof(T)) as IResource<T>
        ...
        /// </summary>
        public static IResource<T> SafeGet => RESTableConfig.ResourceByType.SafeGet(typeof(T)) as IResource<T>;
