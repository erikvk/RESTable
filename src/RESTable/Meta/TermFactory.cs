...
public class TermFactory
{
    ...
    ///     use in conditions.
    /// </summary>
    public Term MakeConditionTerm(ITarget target, string key)
    {
        return MakeOrGetCachedTerm
        (
            target.Type,
    ...
    public Term Parse(Type resource, string key, string componentSeparator, bool isInput, ICollection<string>? dynDomain)
    {
        ...
        foreach (var s in key.Split(componentSeparator)) term.Store.Add(AppendLink(s, dynDomain, term, resource, isInput));
    ...
    {
        if (string.IsNullOrWhiteSpace(key)) throw new InvalidSyntax(ErrorCodes.InvalidConditionSyntax, $"Invalid condition '{key}'");
        if (dynDomain?.Contains(key, StringComparer.OrdinalIgnoreCase) == true) return DynamicProperty.Parse(key);
