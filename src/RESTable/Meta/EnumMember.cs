...
public readonly struct EnumMember
{
    ...
    public static EnumMember[] GetMembers(Type enumType)
    {
        return enumType.IsEnum
            ? enumType.GetFields()
                ...
                (
                    t.Name,
                    (int) (Convert.ChangeType(t.GetValue(null), TypeCode.Int32) ?? -1),
                    t.GetCustomAttributes<Attribute>()
                ))
                .ToArray()
    ...
    /// </summary>
    public bool HasAttribute<T1>() where T1 : Attribute
    {
        return Attributes.OfType<T1>().Any();
    }
    ...
    /// </summary>
    public T1? GetAttribute<T1>() where T1 : Attribute
    {
        return Attributes.OfType<T1>().FirstOrDefault();
    }
}
...
public readonly struct EnumMember<T> where T : Enum
{
    ...
    ///     Gets the members of an enumeration
    /// </summary>
    public static EnumMember<T>[] GetMembers()
    {
        return GetMembers(Array.Empty<T>());
    }
