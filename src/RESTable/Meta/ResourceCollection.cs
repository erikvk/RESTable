...
public class ResourceCollection : ICollection<IResource>
{
    ...
    {
        return ResourceByName.Values.GetEnumerator();
    }
    ...
    {
        return ResourceByName.Values.Contains(item);
    }

    public void CopyTo(IResource[] array, int arrayIndex)
    {
        ResourceByName.Values.CopyTo(array, arrayIndex);
    }

    public bool Remove(IResource item)
    {
        return RemoveResource(item);
    }
    ...
    /// </summary>
    public bool TryGetResource(string name, out IResource? resource)
    {
        return ResourceByName.TryGetValue(name, out resource);
    }

    /// <summary>
    ///     Tries to retrieve the resource with the given type
    /// </summary>
    public bool TryGetResource(Type type, out IResource? resource)
    {
        return ResourceByType.TryGetValue(type, out resource);
    }
    ...
    {
        return ResourceByType.SafeGet(type) ?? throw new UnknownResource(type.GetRESTableTypeName());
    }

    /// <summary>
    ///     Gets the resource for a given type or returns null if there is no such resource
    /// </summary>
    public IResource? SafeGetResource(Type type)
    {
        return ResourceByType.SafeGet(type);
    }
    ...
    {
        return ResourceByName.SafeGet(name) ?? throw new UnknownResource(name);
    }
    ...
    ///     if no resource is found
    /// </summary>
    public IResource? SafeGetResource(string name)
    {
        return ResourceByName.SafeGet(name);
    }
    ...
    /// </summary>
    public IResource<T> GetResource<T>() where T : class
    {
        return ResourceByType.SafeGet(typeof(T)) as IResource<T>
    ...
    /// </summary>
    public IResource<T>? SafeGetResource<T>() where T : class
    {
        return ResourceByType.SafeGet(typeof(T)) as IResource<T>;
    }
    ...
    /// </summary>
    public string GetResourceSpecifier<T>() where T : class
    {
    ...
    ///     if there is no such resource
    /// </summary>
    public ITerminalResource<T> GetTerminalResource<T>() where T : Terminal
    {
        return ResourceByType.SafeGet(typeof(T)) as ITerminalResource<T>
               ?? throw new UnknownResource(typeof(T).GetRESTableTypeName());
    }
    ...
    ///     Gets the terminal resource for a given type or null if there is no such resource
    /// </summary>
    public ITerminalResource<T>? SafeGetTerminalResource<T>() where T : Terminal
    {
        return ResourceByType.SafeGet(typeof(T)) as ITerminalResource<T>;
    }
