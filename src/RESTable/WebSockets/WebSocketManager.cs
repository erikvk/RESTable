...
public class WebSocketManager
{
    ...
    {
        ConnectedWebSockets[webSocket.Id] = webSocket;
    }
    ...
    public async Task HandleTextInput(string wsId, string textInput, CancellationToken cancellationToken)
    {
        ...
        if (await HandleGlobalCommand(terminal, webSocket, textInput, cancellationToken).ConfigureAwait(false)) return;
    ...
    {
        Task sendJson(object obj)
        {
            return webSocket.Send
            (
                JsonProvider.SerializeToUtf8Bytes(obj, obj.GetType(), true, true),
                true,
                cancellationToken
            );
        }
    ...
    public void RemoveWebSocket(string wsId)
    {
        ConnectedWebSockets.Remove(wsId);
    }
}