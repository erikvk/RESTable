...
public static class ExtensionMethods
{
    ...
    ///     The name of the changed property, supplied by the compiler
    ///     (if called from the property set accessor)
    /// </param>
        ...
        where T : class, IPropertyChangeNotifier
    {
        ...
        if (!typeCache.GetDeclaredProperties(typeof(T), true).TryGetValue(propertyName, out var property))
        ...
        (
            target,
            oldValue,
            property.GetValue(target).AsTask().Result
        );
    }
}
