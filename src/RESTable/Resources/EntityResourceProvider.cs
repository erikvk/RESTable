...
///     The base type for all resources claimed by this ResourceProvider.
///     Can be <see cref="object" /> if no such base type exists.
/// </typeparam>
public abstract class EntityResourceProvider<TBase> : IEntityResourceProviderInternal where TBase : class
{
    ...
    IEnumerable<IProceduralEntityResource> IEntityResourceProviderInternal.SelectProceduralResources(RESTableContext context)
    {
        return SelectProceduralResources(context);
    }

    bool IEntityResourceProviderInternal.DeleteProceduralResource(RESTableContext context, IProceduralEntityResource resource)
    {
        return DeleteProceduralResource(context, resource);
    }

    void IEntityResourceProviderInternal.ReceiveClaimed(ICollection<IEntityResource> claimedResources)
    {
        ReceiveClaimed(claimedResources);
    }

    void IEntityResourceProviderInternal.ModifyResourceAttribute(Type type, RESTableAttribute attribute)
    {
        ModifyResourceAttribute(type, attribute);
    }

    bool IEntityResourceProviderInternal.RemoveProceduralResource(Type resourceType)
    {
        return RemoveProceduralResource(resourceType);
    }

    void IEntityResourceProviderInternal.InsertProcedural(RESTableContext context, IProceduralEntityResource resource)
    {
        InsertProcedural(resource, ResourceValidator);
    }

    bool IEntityResourceProviderInternal.Include(Type type)
    {
        return Include(type);
    }
    ...
    {
        foreach (var resource in SelectProceduralResources(null!))
            InsertProcedural(resource, ResourceValidator);
    }

    void IEntityResourceProviderInternal.Validate()
    {
        Validate();
    }
    ...
    /// <summary>
    ///     Creates a new procedural entity resource object with the given name, description and methods. Used by RESTable
    ///     internally. Don't call this method.
    /// </summary>
    ...
    ///     The attribute to use when creating the resource. If null, the attribute
    ///     is fetched from the resource type declaration.
    /// </param>
    /// <returns></returns>
    ...
    /// <param name="attribute">
    ///     The attribute to use when creating the resource. If null, the attribute
    ///     is fetched from the resource's type declaration.
    /// </param>
    ...
    private bool RemoveResource<TResource>() where TResource : class, TBase
    {
        return RemoveResource(ResourceCollection.SafeGetResource<TResource>());
    }
    ...
    {
        return new EntityResource<TResource>
        (
            fullName ?? typeof(TResource).GetRESTableTypeName(),
            attribute ?? typeof(TResource).GetCustomAttribute<RESTableAttribute>() ?? throw new Exception("Could not get RESTableAttribute from resource type"),
            ResolveDelegateSet<TResource, TResource>(delegates),
    ...
    {
        return new EntityResource<TWrapped>
        (
            fullName ?? typeof(TWrapper).GetRESTableTypeName(),
            attribute ?? typeof(TWrapper).GetCustomAttribute<RESTableAttribute>() ?? throw new Exception("Could not get RESTableAttribute from resource type"),
            ResolveDelegateSet<TWrapper, TWrapped>(delegates),
    ...
    private DelegateSet<TResource> ResolveDelegateSet<TTarget, TResource>(DelegateSet<TResource>? delegates)
        ...
        where TTarget : class
    {
        return (delegates ?? new DelegateSet<TResource>())
            .GetDelegatesFromTargetWhereNull<TTarget>()
            .SetDelegatesToDefaultsWhereNull
            (
                DefaultSelect, asyncSelector: DefaultSelectAsync,
    ...
    private View<TResource>[] GetViews<TResource>() where TResource : class, TBase
    {
