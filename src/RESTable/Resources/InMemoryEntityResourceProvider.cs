...
namespace RESTable.Resources
{
    ...
    internal class InMemoryEntityResourceProvider : EntityResourceProvider<object>
    {
        ...
        protected override IEnumerable<T> DefaultInsert<T>(IRequest<T> request) => InMemoryOperations<T>.Insert(request.GetInputEntities());
        protected override IEnumerable<T> DefaultUpdate<T>(IRequest<T> request) => InMemoryOperations<T>.Update(request.GetInputEntities());
