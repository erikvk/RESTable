...
public partial class SetOperations : Dictionary<string, object?>, IAsyncSelector<SetOperations>
{
                    ...
                    switch (argument?.FirstOrDefault())
                    {
                        ...
                        case '/':
                        {
                            ...
                            switch (result)
                            {
                                ...
                                {
                                    await foreach (var entity in entities.ConfigureAwait(false)) yield return jsonProvider.ToJsonElement(entity);
                                    yield break;
                                }
                                case var other:
                                {
    ...
    private static async IAsyncEnumerable<JsonElement> Map(IAsyncEnumerable<JsonElement> set, string mapper, IRequest request, IJsonProvider jsonProvider)
    {
        ...
        foreach (var item in mapped) yield return item;
    }
