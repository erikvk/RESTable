...
{
    /// <summary>
    /// RegEx patterns used internally by RESTable
    /// </summary>
    internal struct RegEx
    {
        ...
        /// </summary>
        internal const string RESTableRequestUri = @"^(?<ignore>\?[^/]*)?((?<res>/[^/-]*)|((?<res>/[^/-]*)(?<view>-\w*)))?(?<cond>/[^/]*)?(?<meta>/[^/]*)?/?$";
        ...
        /// <summary>
        /// Used when sending unescaped data through a RESTable view model
        /// </summary>
        internal const string ViewMacro = @"\@RESTable\((?<content>[^\(\)]*)\)";
