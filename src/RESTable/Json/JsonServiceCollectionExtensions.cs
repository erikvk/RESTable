...
public static class JsonServiceCollectionExtensions
{
    ...
    /// <summary>
    ///     Adds a generic JsonConverter type to be used with RESTable.Json, that will be instantiated for each type that the
    ///     given predicate
    ///     holds for. The instance will be created when the IJsonProvider is instantiated, and any dependencies resolved from
    ///     the service provider.
    ///     The converter will be used for all types that pass the canConvert predicate. If null, the constraints on the
    ///     generic type of the
