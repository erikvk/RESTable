...
public class SystemTextJsonProvider : IJsonProvider, IContentTypeProvider
{
    ...
    {
        return SerializeToUtf8Bytes(item);
    }

    byte[] IContentTypeProvider.SerializeToBytes(object item, Type itemType)
    {
        return SerializeToUtf8Bytes(item, itemType);
    }
    ...
    public async IAsyncEnumerable<T> Populate<T>(IAsyncEnumerable<T> entities, Stream stream, [EnumeratorCancellation] CancellationToken cancellationToken) where T : notnull
    {
        await using (stream.ConfigureAwait(false))
        {
            ...
            if (jsonElement.ValueKind == JsonValueKind.Array)
                switch (jsonElement.GetArrayLength())
                {
                    case 0:
                    {
                    ...
                    default: throw new InvalidInputCount();
                }
    ...
    public T? ToObject<T>(JsonElement element, JsonSerializerOptions? options = null)
    {
        return element.ToObject<T>(options ?? Options);
    }

    public JsonElement ToJsonElement<T>(T obj, JsonSerializerOptions? options = null)
    {
        return obj.ToJsonElement(options ?? Options);
    }

    public object? ToObject(JsonElement element, Type targetType, JsonSerializerOptions? options = null)
    {
        return element.ToObject(targetType, options ?? Options);
    }

    public JsonElement ToJsonElement(object obj, Type targetType, JsonSerializerOptions? options = null)
    {
        return obj.ToJsonElement(targetType, options ?? Options);
    }
