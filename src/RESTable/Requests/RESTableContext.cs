...
public class RESTableContext : IDisposable, IAsyncDisposable, ITraceable, IServiceProvider
{
    ...
    ///     An optional protocol ID, defining the protocol to use for the request. If the
    ...
    public IRequest CreateRequest(Method method = GET, string? uri = "/", Headers? headers = null, object? body = null)
    {
        ...
        if (IsWebSocketUpgrade) WebSocket = CreateServerWebSocket();
    ...
    private static IRequest DynamicCreateRequest<T>(IResource<T> r, RequestParameters p) where T : class
    {
        return new Request<T>(r, p);
    }
    ...
    /// </summary>
    protected virtual WebSocket CreateServerWebSocket()
    {
    ...
    public ValueTask<ReadOnlyMemory<T>> Get<T>(Range range) where T : class
    {
        ...
        return new EntityBufferTask<T>(CreateRequest<T>(), offset, limit, ImmutableList<Condition<T>>.Empty).AsReadOnlyMemoryAsync();
    }
    ...
    {
        var (offset, limit) = range.ToOffsetAndLimit();
        return new EntityBufferTask<T>(CreateRequest<T>(), offset, limit, ImmutableList<Condition<T>>.Empty);
    }
