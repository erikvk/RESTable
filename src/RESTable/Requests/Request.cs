...
internal class Request<T> : IRequest, IRequest<T>, IEntityRequest<T>, ITraceable where T : class
{
    ...
    public async ValueTask<IRequest> GetCopy(string? newProtocol = null)
    {
        ...
        (
            Resource,
            Target,
            newProtocol is not null
                ? protocolController.ResolveCachedProtocolProvider(newProtocol)
                : CachedProtocolProvider,
            Parameters,
            await Body.GetCopy().ConfigureAwait(false),
            MetaConditions.GetCopy(),
            Conditions.ToList(),
            Error
        );
    }
    ...
    public Func<IAsyncEnumerable<T>>? GetCustomSelector()
    {
        return Selector;
    }

    public Func<IAsyncEnumerable<T>, IAsyncEnumerable<T>>? GetCustomUpdater()
    {
        return Updater;
    }
    ...
    public IEnumerable<T> GetInputEntities()
    {
        return GetInputEntitiesAsync().ToEnumerable();
    }
    ...
    {
        return LogItem.GetLogMessage();
    }

    ValueTask<string?> ILogable.GetLogContent()
    {
        return LogItem.GetLogContent();
    }
